...
function isDue(rule: CloneRule): boolean {
  ...
  const now = new Date();
  if (rule.expiry !== "never") { if (now > new Date(rule.expiry)) return false; }
  if (rule.lastRun) {
...
function computeExpiry(option: string): string {
  ...
  const now = new Date();
  const map: Record<string, number> = { "1 Week": 7, "2 Weeks": 14, "1 Month": 30, "3 Months": 90, "6 Months": 180 };
...
function App() {
  ...
  useEffect(() => {
    let a = 0;
    const id = window.setInterval(() => { a++; try { const c = window.TrelloPowerUp?.iframe?.(); if (c) { setT(c); window.clearInterval(id); } } catch {} if (a >= 30) window.clearInterval(id); }, 120);
    return () => window.clearInterval(id);
  }, []);

  // Resize
  useEffect(() => { if (!t?.render || !t?.sizeTo) return; try { t.render(() => { t.sizeTo("#root").catch(() => {}); }); } catch {} }, [t]);
  useEffect(() => { if (!t?.sizeTo) return; const id = requestAnimationFrame(() => { t.sizeTo("#root").catch(() => {}); }); return () => cancelAnimationFrame(id); }, [t, cardMenuOpen, view, cards.length, rules.length, selectedListId]);

  // Fetch board data
  useEffect(() => {
    if (!t) return;
    let cancelled = false;
    async function resolveBoardId() { const c = await t.getContext().catch(() => ({})); if (c?.board) return c.board; const b = await t.board("id").catch(() => null); return b?.id ?? ""; }
    ...
    async function load() {
      ...
      try {
        ...
        const context: Context = pf.context ?? sp.context ?? "board";
        setCtx(context);
        ...
        if (!mem?.username || !bLists?.length || !bCards?.length) {
          try { const { me, ls, cs } = await fetchRest(); mem = { ...(mem ?? {}), ...(me ?? {}) }; if (!bLists?.length && Array.isArray(ls)) bLists = ls; if (!bCards?.length && Array.isArray(cs)) bCards = cs; } catch {}
        }
        if (cancelled) return;
        ...
        if (context === "card" && curCard?.id) {
          ...
          const listId = curCard.idList;
          setSelectedListId(listId);
          const ln = normLists.find((l: TrelloList) => l.id === listId)?.name;
          ...
          if (normLists.length) { setSelectedListId(normLists[0].id); setTargetListName(normLists[0].name); }
        }
  ...
  const cardsInSelectedList = useMemo(() => selectedListId ? cards.filter((c) => c.idList === selectedListId) : cards, [cards, selectedListId]);
  const filteredCards = useMemo(() => {
    const q = cardQuery.trim().toLowerCase();
    const src = cardsInSelectedList;
    if (!q) return src.slice(0, 40);
    return src.filter((c) => c.name.toLowerCase().includes(q) || (listNameById.get(c.idList) ?? "").toLowerCase().includes(q)).slice(0, 40);
  }, [cardsInSelectedList, cardQuery, listNameById]);
  ...
  async function onSave() {
    ...
    setSaving(true);
    try {
      const rule: CloneRule = { id: uid(), srcId: selectedCard.id, srcName: selectedCard.name, listId: targetList.id, listName: targetList.name, repeat, weekday, dayOfMonth: parseInt(dayOfMonth, 10), time: atTime || "09:00", pos: position, expiry: computeExpiry(expiry), active: true, lastRun: null, created: new Date().toISOString() };
      const api = await t.getRestApi(); if (!(await api.isAuthorized())) await api.authorize({ scope: "read,write", expiration: "never" });
      await api.post("cards", { idCardSource: selectedCard.id, idList: targetList.id, pos: position === "Top" ? "top" : "bottom", keepFromSource: "attachments,checklists,comments,labels,members,stickers" });
  ...
  function scheduleLabel(r: CloneRule) { if (r.repeat === "Daily") return `Daily at ${r.time}`; if (r.repeat === "Weekly") return `Every ${r.weekday} at ${r.time}`; return `Monthly on day ${r.dayOfMonth} at ${r.time}`; }
  function expiryLabel(r: CloneRule) { if (r.expiry === "never") return "No expiry"; return `Expires ${new Date(r.expiry).toLocaleDateString()}`; }

  // What to show per context
  const showListSelector = ctx === "board";
  const showCardSelector = ctx === "board" || ctx === "list";
  // card context = settings only
                ...
                <div key={r.id} className={`bg-[#22272B] border rounded-[8px] p-2.5 transition ${r.active ? "border-[#3B444C]" : "border-[#2C333A] opacity-60"}`}>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button type="button" onClick={() => toggleRule(r.id)} className={`h-6 w-9 rounded-full transition relative ${r.active ? "bg-[#579DFF]" : "bg-[#3B444C]"}`}>
            ...
            <>
              ...
              {ctx === "card" && selectedCard && (
                <div className="mb-4 bg-[#22272B] border border-[#3B444C] rounded-[8px] p-2.5">
                  ...
                  <div className="text-[11px] text-[#9FADBC] truncate">{listNameById.get(selectedCard.idList) ?? ""}</div>
                </div>
              )}

              {/* LIST SELECTOR — only for board context */}
              {showListSelector && (
                <SelectField
                  label="Select a list"
                  ...
                  onChange={(name) => {
                    ...
                    if (list) { setSelectedListId(list.id); setTargetListName(name); setSelectedCardId(""); setCardQuery(""); }
                  }}
                />
              )}

              {/* CARD SELECTOR — for board & list contexts */}
              {showCardSelector && (
              ...
              {/* Target list — show for all contexts so user can clone to a different list */}
